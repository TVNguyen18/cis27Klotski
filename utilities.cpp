...
void updatePositionMatrix(int objNumber, int clickedCell, 
	int releasedCell, 	int& modeFlag, int ** posMatrix, 
	Stack * positionStack)
{
	...
	case 1: //the red
		cout << "\nRed object Selected!";
		if (releasedCell == clickedCell + 2 || 
			releasedCell == clickedCell + 6) //moving right
		{
			if (clickedCell % 4 == 2) break; // last column
			if (**(posMatrix + clickedCell + 2) != -1 || 
				**(posMatrix + clickedCell + 6) != -1) 
				//destination cells are not empty
				break;

			//update posMatrix
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell) = 
				*(*posMatrix + clickedCell + 4) = -1;
			*(*posMatrix + clickedCell + 2) = 
				*(*posMatrix + clickedCell + 6) = objNumber;
			break;
		}
		if (releasedCell == clickedCell - 1 || 
			releasedCell == clickedCell + 3) //moving left
		{
			cout << "\nMoving left!";
			if (clickedCell % 4 == 0) break; // first column
			if (*(*posMatrix + clickedCell - 1) != -1 || 
				*(*posMatrix + clickedCell + 3) != -1) 
				//destination cells are not empty
				break;

			//update posMatrix
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell + 1) = 
				*(*posMatrix + clickedCell + 5) = -1;
			*(*posMatrix + clickedCell - 1) = 
				*(*posMatrix + clickedCell + 3) = objNumber;
			break;
		}
		if ((releasedCell == clickedCell - 4) || 
			(releasedCell == clickedCell - 3)) //moving up
		{
			cout << "\nMoving up!";
			if (clickedCell < 4) break; // first row
			if (*(*posMatrix + clickedCell - 4) != -1 || 
				*(*posMatrix + clickedCell - 3) != -1) 
				//destination cells are not empty									
				break;
			...
			//update posMatrix
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell + 4) = 
				*(*posMatrix + clickedCell + 5) = -1;
			*(*posMatrix + clickedCell - 4) = 
				*(*posMatrix + clickedCell - 3) = objNumber;
			break;
		}
		if ((releasedCell == clickedCell + 8) || 
			(releasedCell == clickedCell + 9)) //moving down
		{
			cout << "\nMoving down!";

			if (*(*posMatrix + clickedCell + 8) != -1 || 
				*(*posMatrix + clickedCell + 9) != -1) 
				//destination cells are not empty
				break;
			...
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell) = 
				*(*posMatrix + clickedCell + 1) = -1;
			*(*posMatrix + clickedCell + 8) = 
				*(*posMatrix + clickedCell + 9) = objNumber;
			break;
		}
	...
	case 4: //the yellow
		...
		if (releasedCell == clickedCell + 2) //moving right
		{
			if (clickedCell % 4 == 2) break; // last column
			if ((*(*posMatrix + clickedCell + 2) != -1)) 
				//destination cells are not empty
			...
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell) = -1;
			*(*posMatrix + clickedCell + 2) = objNumber;

			break;
		}
		if (releasedCell == clickedCell - 1) //moving left
		{
			cout << "\nMoving left!";
			if (clickedCell % 4 == 0) break; // first column
			if (*(*posMatrix + clickedCell - 1) != -1) 
			...
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell + 1) = -1;
			*(*posMatrix + clickedCell - 1) = objNumber;
		...
		if ((releasedCell == clickedCell + 4) || 
			(releasedCell == clickedCell + 5)) //moving down
		{
			cout << "\nMoving down!";
			...
			*(*posMatrix + clickedCell) = 
				*(*posMatrix + clickedCell + 1) = -1;
			*(*posMatrix + clickedCell + 4) = 
				*(*posMatrix + clickedCell + 5) = objNumber;
			break;
		}
		// if being in playback mode, return to playing mode
		if (modeFlag == 1)
	...
	case 5: //the green obj
		...
		if ((releasedCell == clickedCell + 1) || 
			(releasedCell == clickedCell + 5)) //moving right
		{
			if (clickedCell % 4 == 3) break; // last column
			if ((*(*posMatrix + clickedCell + 1) != -1) || 
				(*(*posMatrix + clickedCell + 5) != -1)) 
				//destination cells are not empty
				break;
		...
		if (releasedCell == clickedCell - 4) //moving up
		{
			cout << "\nMoving up!";
			if (clickedCell < 4) break; // first row
			if (*(*posMatrix + clickedCell - 4) != -1) 
			...
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell + 4) = -1;
			*(*posMatrix + clickedCell - 4) = objNumber;
			break;
		}
		if (releasedCell == clickedCell + 8) //moving down
		{
			cout << "\nMoving down!";
			if (clickedCell > 15) break; // last row					
			if (*(*posMatrix + clickedCell + 8) != -1) 
				//destination cells are not empty
				break;
			...
			positionStack->push(*posMatrix);
			*(*posMatrix + clickedCell) = -1;
			*(*posMatrix + clickedCell + 8) = objNumber;
	...
	case 9:
		...
		if (releasedCell == clickedCell + 1) //moving right
		{
			cout << "\nMoving right!";
			if (clickedCell % 4 == 3) break; // last column
		...
		if (releasedCell == clickedCell + 4) //moving down
		{
			cout << "\nMoving down!";
			if (clickedCell > 15) break; // last row
			if (*(*posMatrix + clickedCell + 4) != -1)  
